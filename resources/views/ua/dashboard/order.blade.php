                                ...
                                <div class="bg-white rounded-4 shadow-sm mb-5 p-4 p-md-5">
                                    <div class="d-flex justify-content-between flex-wrap mb-4 border-bottom pb-3">
                                        <div>
                                            ...
                                            <small class="text-muted">Waktu Pemesanan: {{ Carbon::parse($order->ordered_at)->format('d M Y, H:i') }}</small>
                                        </div>
                                        <div class="text-end mt-2 mt-md-0">
                                            <span class="badge bg-warning text-dark px-3 py-2">{{ ucfirst($order->status) }}</span><br>
                                            <small class="text-muted">Metode: <strong>{{ strtoupper($order->payment_method) }}</strong></small>
                                        </div>
                                    </div>
                                    <div class="mb-4">
                                        <h6 class="fw-bold">Dikirim ke:</h6>
                                        <p class="mb-1">{{ $order->address->recipient_name }} - {{ $order->address->phone_number }}</p>
                                        <small class="text-muted">{{ $order->address->full_address }}, {{ $order->address->district }}, {{ $order->address->city }}, {{ $order->address->province }}</small>
                                    </div>
                                        ...
                                        @foreach ($order->orderItems as $item)
                                            <div class="d-flex align-items-start gap-3 mb-4 pb-3 border-bottom">
                                                ...
                                                <div>
                                                    ...
                                                    <small class="text-muted">{{ $item->quantity }} x @Rp{{ number_format($item->price, 0, ',', '.') }}</small><br>
                                                    <strong class="text-dark">Rp{{ number_format($item->quantity * $item->price, 0, ',', '.') }}</strong>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
